  ...
  describe '商品の出品' do
    ...
    context '出品できないとき' do
      ...
      it '「発送までの日数」が指定されていないと出品できない' do
        ...
        @item.invalid?
        expect(@item.errors.full_messages).to include("Delivery span can't be blank")
      end
      ...
      it ' 商品の価格は¥300以上でないと出品できない' do
        ...
        @item.invalid?
        expect(@item.errors.full_messages).to include('Product price is out of setting range')
      end
